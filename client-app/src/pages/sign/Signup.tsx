    ...
    <S.SubContainer>
      ...
      <form onSubmit={onSubmitHandler}>
        <S.InputBox color={'#e56717'}>
          <S.Input type='email' value={email} placeholder='이메일' onChange={onEmailHandler} ref={inputRef} />
        </S.InputBox>
        <S.InputBox color={'#e56717'}>
          <S.Input type='text' value={name} placeholder='이름' onChange={onNameHandler} autoComplete='off' />
        </S.InputBox>
        <S.InputBox color={'#e56717'}>
          <S.Input
            type='password'
        ...
        <S.InputBox color={'#e56717'}>
          <S.Input type='password' value={confirmPassword} placeholder='비밀번호 확인' onChange={onConfirmedPassword} />
        </S.InputBox>
        <S.Btn type='submit' color={'#e56717'}>
          회원가입
        </S.Btn>
        {alertToggle ? <AlertModal setAlertToggle={setAlertToggle} alertMessage={alertMessage} /> : null}
      </form>
      <S.Span onClick={() => setToggle(!toggle)} color={'#e56717'}>
        로그인
      </S.Span>
    </S.SubContainer>
  );

...
const CustomToggle = React.forwardRef(
  ({ children, onClick }: { children: React.ReactNode; onClick: React.MouseEventHandler<any> }, ref: any) => (
    <span
      ref={ref}
      onClick={(e) => {
        e.preventDefault();
...
function WishListItem({
  ...
  return (
    <S.Item key={wishListId} style={{ width: '100%' }}>
      <div style={{ float: 'right', marginRight: '10px' }}>
        <Dropdown>
          ...
          <Dropdown.Menu title=''>
            ...
            <Dropdown.Item onClick={() => deleteHandler(wishListId)} style={{ color: 'red' }}>
              삭제
            </Dropdown.Item>
          </Dropdown.Menu>
        </Dropdown>
      </div>
      <S.HeadLine>{wishListName}</S.HeadLine>
      <br />
      ...
      />
      <KakaoMapModal toggle={mapToggle} setToggle={setMapToggle} restaurant={restaurant} />

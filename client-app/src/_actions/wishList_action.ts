...
import { READ_WISHLIST, READ_TEN_WISHLIST, REGISTER_WISHLIST, DELETE_WISHLIST, READ_WISHLIST_COUNT } from './types';
...
export function readWishList(id: string, order: number): { type: string; payload: Promise<WishListType[]> } {
  const request = axios.get(`/api/wishLists?id=${id}&order=${order}`).then((response) => response.data);
...
export function registerWishList(body: {
  user: string;
  username: string;
  name: string;
  address: string;
  created: string;
}): { type: string; payload: Promise<{ success: boolean }> } {
  const request = axios.post('/api/wishList', body).then((response) => response.data);
...
export function deleteWishList(wishListId: string): { type: string; payload: Promise<{ success: boolean }> } {
  const request = axios.delete(`/api/wishList?_id=${wishListId}`).then((response) => response.data);
...
export function readWishListCount(id: string): { type: string; payload: Promise<number> } {
  const request = axios.get(`/api/wishList/count?id=${id}`).then((response) => response.data);

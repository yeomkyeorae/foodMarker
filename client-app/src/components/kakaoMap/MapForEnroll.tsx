...
function MapForEnroll({ toggle, setName, setAddress, width }: Props): React.ReactElement {
  ...
  useEffect(() => {
    kakao.maps.load(() => {
      ...
      function infoWindow(marker, title, itemEl, addrress_name) {
      ...
      function getListItem(index, places) {
        ...
        if (places.road_address_name) {
          itemStr += '<span style="color: white">' + places.road_address_name + '</span>';
        } else {
  ...
  return (
    <div className='map_wrap' style={{ display: 'inline-block', width: '100%' }}>
      <div id={`map`} style={{ width: `${width ? width : '90%'}`, height: '400px', display: 'inline-block' }}></div>
      {alertToggle ? <AlertModal setAlertToggle={setAlertToggle} alertMessage={alertMessage} /> : null}
    </div>
  );
}

export default withRouter(MapForEnroll);
